      ...
      <div className="bg-white rounded shadow overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {q.data?.map((c) => (
              <tr key={c.claim_id} className="border-t hover:bg-ice/30">
                <td className="px-3 py-2 font-mono">
                  <Link className="text-navy hover:underline" href={`/claims/${c.claim_id}`}>{c.claim_number}</Link>
                </td>
                <td className="px-3 py-2">{c.loss_type}</td>
                ...
                <td className="px-3 py-2 text-slate-500">{c.assigned_adjuster ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
